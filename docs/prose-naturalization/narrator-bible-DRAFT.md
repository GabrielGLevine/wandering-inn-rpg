# Narrator / object-prose bible — GH#397 Phase 1 · **DRAFT, for Fable adjudication**

**Status:** DRAFT. Every rule marked **[ADJUDICATE]** is a proposal I could
not settle from evidence alone and that needs Fable's ruling before Phase 2
lanes bind to it. Unmarked rules are supported by the inventory measurements
in `inventory-summary.md` and I am prepared to defend them.

**Scope:** the player-facing prose of `data/maps/**/*.json` — 847 strings,
19,041 words: `observe`, the toast family, embedded `text`
(arrival + ambient), `friendly_line`, `interior_flavor`, `copy`. Exact field
list and every exclusion: the `qa/scripts/extract_prose.py` header.
...
all, and all three shapes are still alive here: **23 CAPS, 6 ellipses, 2 "the
whole of"** — including the exact construction the issue quotes from
`riverfarm_village.json` ("That is the whole of Riverfarm's vocabulary for
gratitude"). The corpus is gated in one register and unguarded in the other.
Rules in §8.

---

## 1. FUNCTIONAL — 224 strings (26%)
...
`interior_flavor`, `unsteady_toast`, `tame_refusal_toast`, the six skill-verb
outcome toasts, `copy`, and any resolution toast that does not bank progress
(a lift ride, a guest cot, `"Your own bed."`).
...
4. **Length ceiling: 2 sentences.** [ADJUDICATE] — chosen because the clean
   examples below are all 1–2 and every over-authored one is 2–3, but this is
   the rule most likely to be too blunt. `copy` (in-world documents) is a
   deliberate exception: those are read objects, and 3–5 sentences is right.

### Corpus examples — this is the sound

- `dungeon/trapped_halls.json · skill_hint_toast` — "Hauling at it idly will
  not do it. Setting your shoulder to it properly might." *Two clauses, the
...
- `pallass/pallass_market.json · locked_toast` — "The brass gate is locked. A
  stencilled plate: PERMIT AND STAMP REQUIRED. The clerk's window is the way
  through this." *Three facts, one of them the answer. CAPS here is diegetic —
  it is what is stencilled on the plate (§8).*
...
## 2. SCENIC — 449 strings (53%), the bulk register
...
5. **Not every prop needs prose worth reading.** Some scenery may be flatly
   informative and forgettable. **[ADJUDICATE]** — I want to state this as a
   quota (e.g. "≥30% of a file's scenic strings carry zero inference") because
   an unquantified permission will be ignored. I do not know if a quota is
   wanted, and it risks becoming the synonym-game the issue warns against.
...
- `riverfarm/witch_hut.json` — **the saturation case, and the one to study.**
  Five props, and three of their toasts run the same inferred microhistory:
  the jars ("Whoever kept this house left it ready and never came back for
  it"), the drying rack ("Somebody kept this place stocked for a house they
  intended to come back to"), the hearth ("It has been cold for years"). The
  hearth's is **protected** — banked ash genuinely implies an intention to
  return, and the issue names it as earned. The other two are the same
  inference with the evidence swapped, and they *retroactively cheapen the
  hearth* by revealing it as the file's template. **Rule: one inferred
  microhistory per map file (§6).**
...
## 3. CHARACTER-BEARING — 165 strings (19%)
...
3. **The narrator does not out-perform the character.** If the sharpest
   observation about an NPC is the narrator's rather than theirs, the NPC has
   been described instead of met. **[ADJUDICATE]** — I believe this rule, but
   it is the least mechanical thing in this document and I cannot give Phase 2
   a test for it beyond the cold read.
...
- `dungeon/trapped_halls.json · toast` — "Behind you, Pisces says nothing,
  which from him is applause." *Named in the issue. Two tells at once: the
  `which from X is Y` shape (12 instances corpus-wide) and the narrator
  supplying the interpretation the player should have made. "Behind you,
  Pisces says nothing" already does the work.*
...
**Currently spent** on 9 strings in 7 of 30 map files — the Anchor
(`dungeon/seal_vault.json`), the seal opening
(`dungeon/trapped_halls.json`), the plinth/Door retrieval
(`ruin/ruin_surface.json`), two Invrisil quest payoffs
(`invrisil/mercantile_alleys.json`), the rift seam, the sewer grate, the
fissure descent.
...
1. **Landmark register is earned by the CHAIN, not by the prop.** A prop does
   ...
   has not been working toward this moment, it is scenic.
...
4. **The evidence still comes first.** `ruin/ruin_surface.json · open_toast`
   ...
   allowance on an *absence*. It is stronger than its sibling variant, which
   spends it on "not treasure. A DOOR —".
...
strings / 7 beats / 30 map files / 847 strings.

| unit | allowance | today |
|---|---|---|
| a map **file** | **≤1 landmark beat** | 6 files at 1, `mercantile_alleys` at 2 |
...
| the **corpus** | **≤12 beats** | 7 spent, **5 held in reserve by Fable** |
...
3. **Reserve is allocated by Fable on petition**, in the dialogue bible's
   idiom (§5d there: 22 grants, 8 held back). A Phase 2 lane that believes a
   beat deserves promotion writes the petition; it does not self-grant.
...
**[ADJUDICATE] — the ceiling of 12.** 7 spent + 5 reserve is a judgement. The
argument for 12: it keeps landmark at ~1.5% of map prose, so a player meets
one roughly every four maps, and the 50 demotion candidates cannot all be
absorbed by promotion instead of rewriting. The argument against: Act V and
the Invrisil chains may genuinely want more, and a ceiling set from the
current state ratifies the current state. I recommend 12 and flag that I am
guessing at the numerator.

**[ADJUDICATE] — whether `victory_toast` should be landmark-eligible at all.**
Two of the three are quest payoffs and read as landmark; the third is Relc
talking (hand-demoted to character-bearing). Post-combat is a moment the
player has *worked* for, which argues for eligibility; it is also the most
template-prone surface in the game, which argues against.
...
anonymous agent, against 183 of 1,438 (13%) in dialogue. And it inverts with
population:
...
| ruin | 47% | nobody |
| sewers | 44% | nobody |
| invrisil · pallass · riverfarm · inn · liscor | 26–30% | populated |
...
4. **Corpus target: ≤12% of map strings**, i.e. ~100, down from 267 — parity
   with dialogue's 13%, since criterion 6 requires map prose to end no worse
   than dialogue. Advisory metric, not a hard gate (the issue forbids new
   synonym quotas): **[ADJUDICATE]** whether 12% is the right number and
   whether the unpeopled regions should carry a *stricter* cap than populated
   ones. My instinct is stricter — the emptiness is the setting's content and
   inferring inhabitants spends it — but the opposite case is real: a ruin's
   only available characterisation *is* its absent builders.

---

## 7. Negation / correction, at the distribution level

**Measured:** 29 instances in map prose (3%), 11 in dialogue (1%). The issue's
instruction is to reduce this **at the distribution level** and keep it where
it is voice-distinctive.
...
distinctive *of a speaker*. So the keep criterion has to be re-derived, and I
propose it be **function**, not voice:
...
Worked through the issue's own examples:
...
- `ruin/ruin_surface.json · open_toast` — "Below the pedestal: not treasure. A
  ...
  treasure is real and the scene set it up. CAPS goes.
- `hedault_enchanting.json` (dialogue, cited by the issue) — not this bible's
  jurisdiction; noted so Phase 3 sees the parallel.
...
1. **Zero in functional register.** A gate refusal never corrects an
   ...
   not.").
2. **Corpus ceiling for map prose: 8**, down from 29. **[ADJUDICATE]** — the
   dialogue bible's comparable ceiling is 30 across 56 speakers; 8 across one
   narrator is the proportional read, but I am inferring the proportion.
3. **At most one per map file**, and never twice on the same prop.
...
prose. They are the cheapest wins in the pass.
...
| CAPS emphasis | ~8 of 23 | **0** for emphasis. |
| CAPS as **diegetic lettering** | ~15 of 23 | **PERMITTED.** |

**The CAPS split is the important part, and is [ADJUDICATE].** Roughly 15 of
the 23 instances are rendering what is physically written on an object —
`COYLE AND SONS` on a hanging board, `PERMIT AND STAMP REQUIRED` on a
stencilled plate, `THE WANDERING INN` on a hand-painted sign, `WATCH NOTICE` /
`TRAVELER'S NOTE` heading the Guild board's `copy` documents. Those are
correct: the capitals *are* the object. Only ~8 are emphasis — `ANCHOR`,
`CLICKS`, `POURING`, `HAND`, `SLEEP`, `RECOVERS`, `DOOR`, `WITH`.

A naive port of the dialogue gate's `\b[A-Z]{3,}\b` rule would fail 15 correct
strings. I recommend the rule be **"CAPS only for lettering the player is
looking at"**, and that Phase 4's advisory metric report the two classes
separately rather than gate on the raw count. What I cannot settle: whether a
mechanical test exists (signage CAPS tends to follow a colon or a comma-plus-
appositive: "A stencilled plate: PERMIT AND STAMP REQUIRED") or whether this
has to stay a judgement call.
...
| **motion** | `sewers/deep_tunnels.json · taken_toast` — "The bones rise and fall into step behind you." |
...
2. **Within one map file, no single ending shape may exceed 40% of its
   scenic + functional strings.** **[ADJUDICATE]** on the number — 40% is
   chosen so a small file (10–15 strings) is not forced into artificial
   variety, but I have not tested it against the current corpus per file.
3. **Within a region, at least 4 of the 6 shapes must appear.**
4. **Two adjacent props may not share an ending shape.** Adjacency is the
   scale a player actually experiences — they interact with a shelf and then
   the bench beside it — and the issue's Phase 4 list names "sentence-ending
   similarity across neighbouring strings" for exactly this reason.
   **[ADJUDICATE]**: "adjacent" needs a definition. Same entity list order?
   Grid proximity? I lean on **grid proximity within ~3 cells**, since that is
   what the player walks, but entity order is far cheaper to compute.
...
1. **Not every string changes.** The work queue is 50 demotion candidates out
   of 847, plus the 8/6/2 inherited-ban instances and the anonymous-agent
   reduction. The default action on a scenic string is **leave it alone**. A
   region lane that rewrote most of its strings has misread the assignment.
2. **The protected keeps are calibration, not exceptions.**
   `protected-keeps.json` carries MODEL entries specifically so lanes have
   in-corpus examples of the target sound. Read them before starting.
...
4. **Sentence-length variance is a first-class check.** A file whose strings
   all land in 12–18 words has been templated even if each is individually
   fine. **[ADJUDICATE]** — I would like a reported per-file
   sentence-length standard deviation in Phase 4's advisory metrics, and I do
   not know whether that is a metric worth its weight or another number to
   game.
...
## 11. Open [ADJUDICATE] register

Every ruling Fable needs to make, collected:

| § | question | my recommendation |
|---|---|---|
| 1.4 | functional length ceiling of 2 sentences (`copy` excepted) | adopt, expect to loosen |
| 2.5 | quantify "some scenery may be forgettable" as a zero-inference quota? | adopt a soft target, do not gate |
| 3.3 | "narrator does not out-perform the character" — untestable by tooling | keep as a read-only principle |
| 5 | landmark corpus ceiling of 12 beats (7 spent, 5 reserve) | adopt 12 |
| 5 | is `victory_toast` landmark-eligible? | yes, with the Relc demotion standing |
| 6.4 | anonymous-agent target of ≤12%; stricter in unpeopled regions? | adopt 12%; **stricter** in ruin/dungeon/sewers/garden |
| 7.2 | negation/correction map ceiling of 8 (from 29) | adopt 8 |
| 8 | CAPS: permit diegetic lettering, ban emphasis — mechanically separable? | permit by judgement, report both classes separately |
| 9.2 | 40% single-ending-shape cap per file | adopt, verify against current per-file data first |
| 9.4 | definition of "adjacent" for ending-shape collision | grid proximity ~3 cells; accept entity order if cost matters |
| 10.4 | per-file sentence-length variance as an advisory metric | adopt if cheap, drop if it invites gaming |

**Two things I deliberately did NOT decide, because they are Fable's:**

- Whether the dialogue bible's tier system should extend to map prose at all
  (I argue in §0 that it should not, and that registers replace it).
- Whether any of this becomes a hard gate. The issue's Phase 4 is explicit
  that these are advisory metrics first and that turning them into synonym
  quotas is a failure mode. Nothing in this document should be read as
  proposing a new red.
