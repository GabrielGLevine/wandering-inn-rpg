...
- [ ] Add failing contracts for persistent overlay ownership, 0.125+0.125
  timing, full-cover rebuild ordering, QA collapse, visual opt-in, Main-wide
  input consumption, and World movement gating.
- [ ] Implement the persistent CanvasLayer/ColorRect and async
  `transition_map(rebuild)` seam.
- [ ] Route only `MAP_CHANGED` rebuilds through it; keep initial world build
  synchronous.
- [ ] Run world visual/unit, load gate, and representative crossing scripts.
- [ ] Commit and independently review; fix all Critical/Important findings.
...
- [ ] Add a seed-9 full-tier inn-to-street crossing canonical with midpoint
  and post-reveal screenshots; register and derive surfaces.
- [ ] Prove headless collapse plus unchanged crossing scripts.
- [ ] Run it windowed with `--map-transition-visual`; read both frames.
