...
- [ ] **(P2)** | Hedault workshop night grade | invrisil interiors | World-band mean luminance 22/255, p90 32 — roughly half the next-darkest night scene; floor/wall/NPC all indistinguishable | v0.17 close machine-playtest, invrisil_setting_talk/03 |
...
- [ ] **(P2)** | Invrisil boulevard plaza fill | invrisil_boulevard | Flat untextured #424FA3 rectangle, razor edge against cobbles, brighter than the lamplit street; no texture unlike the riverfarm water tile | v0.17 close machine-playtest, stationer_room_loop/04 |
- [ ] **(P2)** | Toast vs field-skill legend | all maps, HUD | Toast panel occupies the same band as the legend and is drawn over it, truncating legend text mid-word ('...and files i') | v0.17 close machine-playtest, stationer_room_loop/02 |
- [ ] **(P2)** | HUD chip bar | player room, den-shop, inn | Inventory/Journal/Pause chips absent for the whole first visit to the purchased room (chip-region max 53 vs 255 after a pause cycle) with no visible modal on screen; touch has no fallback | v0.17 close machine-playtest, player_room_loop/03+04 |
