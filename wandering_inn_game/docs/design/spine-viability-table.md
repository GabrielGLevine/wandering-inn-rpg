...
| I | `act1_gate_ambush` | w1 / spear | WIN 0.68 / 4 rd / +11 | WIN 0.73 / 4 rd / +13 | None authored. Proximity trigger on the only road to the gate; Relc fields off `met_relc`.<br>_proximity r2 (sneak suppresses)_ |
...
| II | `act2_cistern_nest` | w1/m1 / spear | LOSS 0.03 / 4 rd / -63 | LOSS 0.42 / 4 rd / -7 | Scout path resolves `resolved_the_cisterns` without the nest (shipped run's choice). Klbkch fields off `chatted_with_klbkch`.<br>_interact-only_ |
| III | `act3_raskghar_scouts` | w2/m1 / spear | LOSS 0.06 / 3 rd / -35 | WIN 0.50 / 3 rd / +1 | None authored — solo, no ally slot, and the scouts sit on the only lane east to the warren mouth.<br>_interact-only_ |
| III | `act3_awakened_boss` | w2/m1 / spear | LOSS 0.18 / 5 rd / -44 | WIN 0.68 / 5 rd / +15 | None (act climax). `relc_descent` is a veto beat, not an out: [Go together] banks `relc_joined_descent`, which is the boss's `ally_requires`. This is the JOIN route — Awakened + 2 scouts + a Sewer Bat, with Relc — and the veto route is no longer inferred from it: it is measured, one row down, off the entity's own `solo_enemies`.<br>_interact-only_ |
| III | `act3_awakened_boss_solo` | w2/m1 / spear | LOSS 0.00 / 3 rd / -61 | LOSS 0.45 / 4 rd / -2 | None (act climax), and no ally: this is the [I go alone.] branch. RULED WINDOW, not the reference window — the fight is DESIGNED to sit under 0.5, so the five-climax [0.55, 0.85] band would be the wrong gate and `SPINE_CLIMAX_IDS` deliberately omits it. The floor column is expected LOSS-heavy and is report-only; competent-at-band is the contract.<br>_interact-only_ |
| IV | `act4_vault_construct` | w11/m2 / spear | WIN 0.82 / 7 rd / +52 | WIN 0.82 / 6 rd / +52 | None (act spine). The Horns field off `horns_party_formed`; the plates-guidance fork costs Ksmvr 18 max HP (`ally_hp_penalty`), modelled here.<br>_gated on `halls_cleared` + `horns_party_formed`, interact-only_ |
| IV | `act4_ruin_guardian` | w11/m2 / spear | WIN 0.77 / 4 rd / +24 | WIN 1.00 / 3 rd / +44 | The dig has a non-fight route to `pedestal_breached`; this row is the fight leg. Relc fields off `met_relc`.<br>_interact-only_ |
| IV | `act4_crypt_lich` | w11/m2 / spear | LOSS 0.18 / 4 rd / -38 | WIN 0.85 / 4 rd / +23 | FULLY OPTIONAL: interact-only (no trigger_radius), on no route and on no existing pin, so declining it costs nothing but the levels it pays. It does not respawn -- one raising of this crypt per save.<br>_interact-only_ |
| IV | `act4_alley_footpads` | w12/m2/d7 / spear | WIN 1.00 / 2 rd / +41 | WIN 1.00 / 2 rd / +43 | Sneak-shaped only: proximity trigger, suppressed while sneaking. Ledger finding 4 — 'the alleys cannot be crossed clean without [Stealth]', and the spine build eats TWO of these across four crossings.<br>_proximity r2 (sneak suppresses), respawns (one per sleep)_ |
| IV/V | `act5_gallery_vermin_nest` _(gold-scaled)_ | w12/m2/d7/t2 / spear, 52 HP | LOSS 0.27 / 4 rd / -24 | WIN 0.63 / 3 rd / +14 | Optional side gallery — interact-only, never triggers on approach. `respawns: true` = one grind per sleep. THE AUTOPLAY WALL: hand-winnable, autoplay-losable.<br>_gated on `halls_cleared`, interact-only, respawns (one per sleep), rank-scaled_ |
| V | `act5_seal_warden` | w12/m2/d7/t2 / spear, 52 HP | LOSS 0.32 / 5 rd / -24 | WIN 0.54 / 5 rd / +2 | Alcove sneak-past (trigger_radius 1, suppressed while sneaking) — reachable ONLY with a sneak-shaped ability. Ledger finding 6: [Sneak]/[Hedge Remedy]/[Detect Magic] are all off this build's tree; the run reached the fork by EQUIPPING the moon-bone amulet for its [Invisibility]. Per CHOICE-LOG 2026-08-11 that bypass is a defect (#440), not a feature.<br>_gated on `read_the_feeding_ward`, proximity r1 (sneak suppresses)_ |
| V | `act5_seal_warden_amulet` | w12/m2/d7/t2 + moon-bone amulet | LOSS 0.49 / 5 rd / -1 | WIN 0.70 / 5 rd / +7 | SHIP COLUMN: the same fight with the carried-but-unequipped upgrade WORN — hp+3, dmg+1, and [Invisibility] into the kit, which the competent policy spends as an escape below 35% HP. BAND COLUMN: no amulet (its resonance would not fit beside the band build's two charms) — this is the band TOP read, spellsword 16.<br>_gated on `read_the_feeding_ward`, proximity r1 (sneak suppresses)_ |
...
|---|---|---|---|---|---|
| I | `act1_gate_ambush` | `spellspear` | warrior2 | WIN 0.85 / 4 rd / +20 | IN WINDOW |
| II | `act2_cistern_nest` | `spellspear` | warrior3/mage2 | WIN 0.72 / 4 rd / +24 | IN WINDOW |
| III | `act3_awakened_boss` | `spellspear` | warrior5/mage4 | WIN 0.76 / 4 rd / +26 | IN WINDOW |
| IV | `act4_vault_construct` | `spellspear` | warrior7/mage6 | WIN 0.83 / 7 rd / +53 | IN WINDOW |
...
| I | `act1_gate_ambush` | `skirmisher` | warrior2 | WIN 0.85 / 4 rd / +20 | IN WINDOW |
| II | `act2_cistern_nest` | `skirmisher` | warrior3/archer2 | LOSS 0.28 / 4 rd / -32 | WALL — report only |
| III | `act3_awakened_boss` | `skirmisher` | warrior5/archer4 | WIN 0.74 / 5 rd / +19 | IN WINDOW |
| IV | `act4_vault_construct` | `skirmisher` | warrior7/archer6 | WIN 0.84 / 6 rd / +53 | IN WINDOW |
...
|---|---|---|
| shipped run — w12/m2/d7/t2 / spear, 52 HP | LOSS 0.32 / 5 rd / -24 | WIN 0.54 / 5 rd / +2 |
| shipped run + moon-bone amulet worn | LOSS 0.49 / 5 rd / -1 | WIN 0.70 / 5 rd / +7 |
...
| `act5_gallery_vermin_nest` | ship | competent | WIN | ✅ WIN 0.63 / 3 rd / +14 | measured: hand-winnable — the gap IS the competence gap |
...
6. **The gallery nest is a pure competence wall.** Floor LOSS 0.27 / 4 rd / -24, competent WIN 0.63 / 3 rd / +14 at the same build and the same (empty) pack. Nothing about the encounter is unfair; the script simply cannot play it. It is also `scales: true`, and the Act V build's effective power puts it in the GOLD band (+50% enemy HP, +2 damage) — the trash got a rank promotion the run never noticed.
