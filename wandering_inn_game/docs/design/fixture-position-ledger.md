...
`post_game` is banked by the SLEEP PASS (`src/core/sleep_beat.gd`), silently,
at the first sleep after `raskghar_sealed` — no epilogue required (2026-07-26
reframe; the epilogue's own `_bank_post_game` in `sleep_veil.gd` stays on as an
idempotent twin until the finale phase retires it, so whichever fires first
wins and the second is a no-op). It is Act IV's own `counted_among` beat
condition (`data/acts.json`; that beat lived on act_iii as `seal_holds` before
the same reframe).
