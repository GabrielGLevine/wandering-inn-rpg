...
WHAT THIS IS
    ...
    Every artifact under wandering_inn_game/qa/baselines/prose/ except the two
...
# plus a one-word rationale tag; wandering_inn_game/qa/baselines/prose/classification-
...
def write_inventory_summary(rows, classified, keeps, hold_ids, ranked, outdir):
    ...
    L = ["# Prose inventory + register classification — GH#397 Phase 0/2 prep\n",
         gen_note("all --outdir qa/baselines/prose") + "\n",
...
RULED_EXC_SOURCE = ("docs/superpowers/specs/2026-08-05-wave2-rulings-and-fix-specs.md "
...
def report_data(docdir):
    ...
    D = {"_python": PY, "_seed": SEED,
         ...
         "_pins": {"protected_keeps": len(keeps), "holdout": len(hold),
                   "source": "wandering_inn_game/qa/baselines/prose/protected-keeps.json + "
...
def render_report(D):
    L = ["# Phase 4 — advisory distribution metrics — GH#397\n",
         gen_note("report --outdir qa/baselines/prose") + "\n",
    ...
    L += ["", "---\n",
          "## 11. Reproducing this\n",
          "```sh\n"
          "python3 qa/scripts/extract_prose.py report --outdir qa/baselines/prose\n"
...
def main():
    ...
    print(f"'{a.cmd}' is produced by `all` (shared derived state); run: "
          f"extract_prose.py all --outdir qa/baselines/prose", file=sys.stderr)
    return 2
