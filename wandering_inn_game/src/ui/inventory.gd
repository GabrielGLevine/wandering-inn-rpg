...
## every mechanical detail (effect lines, lore, description) lives in a
## SELECTION-DRIVEN detail column beside the list (`_detail_box`, built in
## `_ready()`'s HBox `body`), refreshed by `_render_detail()` on every
## cursor move / rebuild. Lore reads as flavor purely through PLACEMENT
## (its own row, between the effect lines and the description) and STYLING
## (the `"Lore"` theme type variation, wi_ui_theme.tres -- dimmer/
...
## CARRIED item in list order) is independent of how/where the effect lines
## are drawn on screen.
##
...
## mechanical effect lines / lore / description live in the
## selection-driven `_detail_box` (see `_render_detail`, called at the end
## of this function so the two columns can never drift out of sync). After
## rebuilding, scrolls the cursor's row into view (see `_scroll`'s doc
## comment).
...
## on-parchment precedent as dialogue_panel.gd's speaker label), the
...
## effect lines and the "Lore" theme type variation's dimmer styling), and
## the wrapped description prose. Called at the end of every
...
func _render_detail() -> void:
	...
	# The mechanical effect lines, GENERATED from the item's data via the
	# shared WIEffectText formatter (never hand-composed here -- that drift
	# is the defect this kills). item_effect_lines already ends with the
	# "Worth N gold" value where the item is priced. A plain item (no mods,
	# no price) yields an empty array -> no effect rows.
	for effect_line: String in WIEffectText.item_effect_lines(rec):
		_detail_box.add_child(UIChrome.make_label(effect_line))
	# The lore line, UNLABELED (no "Lore — " prefix) -- the "Lore" theme
	# type variation (wi_ui_theme.tres) is dimmer/desaturated relative to
	# both the solid dark effect-line text above and "Small"'s
