...
func _init() -> void:
	...
	assert(armed == ["lead_dig", "lead_witch_ear", "lead_flood_ledger", "lead_winter"], "leads: spine_started arms the witch capstone (and retires lead_spine, its own hide_when)")

	leads.accomplishments["lattice_witch_lore"] = 1
	var spent: Array = []
	for raw_lead: Variant in leads.active_leads():
		spent.append(String((raw_lead as Dictionary)["id"]))
	assert(spent == ["lead_dig", "lead_flood_ledger", "lead_winter"], "leads: taking the capstone conversation retires its lead")
