...
##
## `tests/sim_spine_viability.gd` is the REPORT: twelve spine fights x two build
## families x two policies x 100 seeds, and it regenerates
## `docs/design/spine-viability-table.md`. It is named `sim_` on purpose, so
...
func _init() -> void:
	...
	if not _failures.is_empty():
		...
		quit(1)
		return
	print("PASS: %d calibration rows hold and %d retuned climaxes sit inside the #439 tuning window" % [
		(SPINE.CALIBRATION as Array).size(), RETUNED.size()])
	quit(0)
