...
`docs/archive/design/invrisil-art/pixellab-batch.md`; per-job `job_*.json` provenance
...
## 8e Phase A batch (2026-07-12) — Pallass tile family + Grimalkin/tier-clerk rigs

**Tile family** (2 owned tilesets, generated by the 2026-07-07 pallass-art
direction lane's `/create-tileset` pass, zero new generations this task —
promoted from the gitignored cache to committed files; full verdict +
in-hand-packs-first reasoning: `docs/archive/design/pallass-art/tile-family-verdict.md`):

| Path | Subject | Notes |
|---|---|---|
| `assets/tiles/pallass/tileset_slate_over_void_atlas.png` (+`_meta.json`) | Market-tier engineered slab floor / void-edge parapet | 64×64 = 16 Wang tiles of 16px |
| `assets/tiles/pallass/tileset_brick_over_molten_atlas.png` (+`_meta.json`) | Forge-tier dark brick floor / molten-channel edge | 64×64 = 16 Wang tiles of 16px |
| `assets/sprites/{great_lift,crystal_lamp,steam_vent,price_board,forge_station,market_stall_pallass,tier_wall}/Idle-Sheet.png` | Pallass landmark props (elevator, lamp, vent, price board, forge station, market stall, wall facade) | Committed but NOT wired into `sprites.json` this pass — placement/scale wants real map context (Phase B); measured anchors recorded in `tile-family-verdict.md` for that pass to consume directly |

**Grimalkin + tier-clerk character rigs** (v2 `create-character-pro`,
`mannequin`/`low top-down`, SEQUENTIAL per the ops rule, `animate-character`
template mode `breathing-idle`(4)/`walking`(6), directions restricted to
south/east/north only — cheaper AND matches the down/side/up shape this repo
actually consumes):

| Path | Subject | Provenance |
|---|---|---|
| `assets/sprites/grimalkin/{Idle,Walk}_{Down,Side,Up}-Sheet.png` | Grimalkin ([Sinew Magus], Pallass Magic-Captain) | pro char `82cb307c-2a38-42df-8a7e-f35f352fd182` ("massive hyper-muscular Drake warrior-scholar, bright green scales, tight-fitting clothing straining..."); requested 132×132, exported 224×224 frames (pro-mode ~2x canvas padding). NOTE: a first create (char `aabdb3d0`, same prompt) reported completed but its rotation files never landed on storage — every animate against it failed server-side ("Could not fetch frozen frame", zero generations charged) — deleted + recreated; this id is the good one. |
| `assets/sprites/tier_clerk/{Idle,Walk}_{Down,Side,Up}-Sheet.png` | Tier clerk (ORIGINAL+flag civic role, every Pallass stamp-desk) | pro char `77b48a16-cadc-496a-9b47-fbd36bfd91a7` ("trim Drake bureaucrat clerk, stiff rectangular formal coat, stamp+ledger..."); requested 108×108, exported 140×140 frames. |

Full prompts (derived from `character-profiles.md`'s Grimalkin/Tier-clerk
entries per the profiles-derive-prompts rule), measured anchors, and
render_scale derivation: see those two `character-profiles.md` entries +
the `sprites.json` `_comment` field on each rig. Same PixelLab-ToS
ownership / redistributability as everything above.
