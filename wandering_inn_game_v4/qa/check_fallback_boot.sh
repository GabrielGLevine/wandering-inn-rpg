...
# else (no untracked dev tooling like the godot-ai `addons/`, no .godot import
...
run_step() {
	...
	# Grep discipline: fallback_art lines are EXPECTED here and exempt; the
	# godot_ai game_helper capture line is exempt as in ci_sweep.
	local hits
	hits="$(grep -nE 'SCRIPT ERROR|Parse Error|WARNING' "$log" \
		| grep -v 'godot_ai game_helper' \
