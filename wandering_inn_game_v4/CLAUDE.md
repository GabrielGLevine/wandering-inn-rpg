...
## Commands
	...
	wandering_inn_game_v4/qa/ci_sweep.sh                           # all 43 at pinned seeds + grep discipline; --only a,b,c to restrict; list MIRRORS the seed table below — keep in sync
...
and `d2_shop_shot`)**),
pinned straightaway (no seed search needed unless noted): run these 43 headless
...
| `deep_descent` | 9 | **M-ARC A2 (new canonical, +1 → 43).** THE Raskghar descent proof — fixture-based via `deep_descent_start` (sewers, warrior L5, `heard_the_deep_tremor` pre-banked, facing the `deep_fissure` at (8,12); fixture `rng_state` governs the scout autoplay, overrides `--seed`). Interact the fissure → **descend** to `deep_tunnels` (open_toast + `map_changed{deep_tunnels}`, land (2,2)) → read Cold Hearth flavor prop → clear the interact-only `raskghar_scouts` route-fight (2× `raskghar_scout`, arena `cave_mouth`, `combat_autoplay`, NO ally — the party/veto is A3) → read Gnawed Bones flavor prop → reach `warren_mouth` (the "warren door", terminal beat). **A3 fixture-note:** the Awakened BOSS fight (`deep_warren` arena) rides A3's chain — A3 wires the boss encounter at/beyond `warren_mouth` + the Relc party/veto; this script STOPS at the warren door. **NEW GATE KEY `heard_the_deep_tremor`:** the `deep_fissure` door_when gate; A3's tremor beat (Zevara's post-Act-II summons) is its SOLE producer — pre-gate the fissure is byte-identical sewers behavior (banks `found_the_fissure` + toast), so `sewers_walkthrough`/`cisterns_*` stay green (the fissure sits at (8,12), the untouched bottom-middle band). |
