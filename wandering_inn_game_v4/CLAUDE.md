...
## Commands
	...
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_dialogue.gd
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_quests.gd
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_save.gd
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_content.gd
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_sprite_registry.gd
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_combat_visuals.gd
	/usr/local/bin/godot --headless --path wandering_inn_game_v4 --script res://tests/test_audio_data.gd
	...
	wandering_inn_game_v4/qa/run_qa.sh level_up_loop headless     # ONBOARDING O5: post_tutorial FIXTURE loop — fixture rng 9 overrides --seed (see the seed table row)
	wandering_inn_game_v4/qa/run_qa.sh mage_unlock_loop headless --seed=9        # scroll -> sleep -> [Mage] gained -> fight with mage kit fielded
	wandering_inn_game_v4/qa/run_qa.sh line_of_sight_denial headless --seed=9    # wall-aware ranged AI (goblin_ambush walls); asserts the positive has_los gate -- no "denial" event fires (see Gotchas; archived note: .superpowers/sdd/archive-m2-m3-task-docs/task-8-report.md)
	wandering_inn_game_v4/qa/run_qa.sh lantern_check headless --seed=9           # scroll -> sleep -> [Mage]/light gained -> unlit_lantern skill_used + lit_the_common_room + toast
	wandering_inn_game_v4/qa/run_qa.sh defeat_reload headless --seed=1           # losing chieftains_raid seed; asserts defeat reloads auto slot instead of reset
	wandering_inn_game_v4/qa/run_qa.sh defeat_ally_alive headless      # ONBOARDING O5: near_defeat FIXTURE (rng 3 overrides --seed) — THE canonical proof of the PC-death defeat rule: PC drops while Relc is ALIVE (hp==40 pinned) => instant defeat + auto-slot reload, no game_reset
	wandering_inn_game_v4/qa/run_qa.sh combat_move_input headless --seed=9       # M5 H2 movement-first: arrows step the unit directly, Dash refills pool; ends mid-combat by design
	...
	wandering_inn_game_v4/qa/run_qa.sh relc_tutorial headless --seed=9           # negative decline (no combat_started) -> real spar, real-key beat-by-beat tutor_lines, then opacity/persistence teeth + re-talk second combat_started; ends mid-fight by design (combat_move_input precedent)
	...
	wandering_inn_game_v4/qa/web/run_web_qa.sh inn_walkthrough              # add --skip-export to reuse last build
	wandering_inn_game_v4/qa/web/run_web_qa.sh combat_walkthrough 9         # seed is threaded through window.__WI_QA__ -> JavaScriptBridge (M3 T8)
...
- **Combat depth (M3):** movement economy — `move_pool` (3 free steps/turn) spends
  ...
  **Verdict: no item numbers moved.** `warrior2_sword` reproduces the plain
  ...
  (seed 9) and `combat_walkthrough` (seed 9) both green post-harness-edit.
	...
	wandering_inn_game_v4/qa/run_qa.sh dialogue_walkthrough headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh dialogue_hub_loop headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh quest_errand_fight headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh quest_errand_parley headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh save_load_roundtrip headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh combat_walkthrough headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh level_up_loop headless   # ONBOARDING O5: now a post_tutorial FIXTURE loop (title Continue); fixture rng 9 overrides --seed
	wandering_inn_game_v4/qa/run_qa.sh mage_unlock_loop headless --seed=9
	wandering_inn_game_v4/qa/run_qa.sh line_of_sight_denial headless --seed=9
	...
	wandering_inn_game_v4/qa/run_qa.sh gate_district_walkthrough headless --seed=9   # enters street for real from the floodplains liscor_gate; y4 transit lane + krshia_stall/guild_door/sewer_grate (exact toasts) + gate Watch guard (dialogue_line) + Selys (26,4); owns the map_changed{street}->audio_played{music_street} assertion combat_walkthrough dropped
	# Playtest-content slice T3: "The Missing Crate" (street, 3 solution paths -- fight/talk/skill)
	wandering_inn_game_v4/qa/run_qa.sh crate_fight headless --seed=9    # FORCE path: fights the new crate_scavengers encounter (2x goblin_raider, goblin_ambush arena, no ally) via combat_autoplay
	wandering_inn_game_v4/qa/run_qa.sh crate_talk headless --seed=9     # WATCH path: persuades the new watch_sergeant to clear crate_scavengers, no combat_started anywhere
	wandering_inn_game_v4/qa/run_qa.sh crate_light headless --seed=9    # SKILL path: [Light]-studies the new cellar_door prop; proves the hidden-until-met report option is absent before studying
	wandering_inn_game_v4/qa/run_qa.sh journal_skills headless --seed=9  # UI wave item 19: journal skills-by-class panel -- pre-first-use NAME ONLY, post-first-use full description, grouped-by-class payload structure

	# M7 Task E5: inventory/equipment loop
	wandering_inn_game_v4/qa/run_qa.sh inventory_loop headless --seed=9  # ONBOARDING O5: chest pickup -> spar -> re-talk Relc for the GIFT node (spear, O3 moved it off the spar-accept) -> sleep Warrior L1 -> equip the spear via the real UI -> cross the goblin_encounter_1 ambush (30,23) proving the weapon-gated kit (piercing_strikes IN, power_strike OUT) -> journal lists both (knowledge != fieldability)

	# M-BEAUTY Task B1: mood grade + phase-clock consumer
	wandering_inn_game_v4/qa/run_qa.sh atmosphere_check headless --seed=9  # world_ready -> ui_mood_applied{inn,day} -> journal open/close under identity grade (UI-ungraded proof) -> 40 real actions (paced back-and-forth moves) -> phase_changed{dusk} + ui_mood_applied{dusk} -> sleep at bed -> phase_changed{day} + ui_mood_applied{inn,day}
	...
	wandering_inn_game_v4/qa/run_qa.sh tutorial_flow headless --seed=9
...
- **Equipment (M7, 2026-07-05):** pure sim state — `WIGame.inventory`
  ...
  fold): `phase()` = pure function of `actions_since_sleep` (dusk ≥40,
  night ≥90, config-injected), `phase_changed` on crossings + sleep
...
- **Atmosphere / mood grade (M-BEAUTY Task B1, 2026-07-05):** presentation-only,
  ship-neutral-first. `data/moods.json` (`meta.phase_thresholds` {dusk:40,
  night:90} is LIVE as of the M-BEAUTY final-review fix wave (Fix 3,
  ...
  itself was untouched by the fix). The shipped values still MATCH
  `wi_game.gd`'s own hardcoded `_phase_config` defaults, so wiring it was
  behavior-neutral (zero seed shift — verified: `atmosphere_check` + 2
  pinned-seed combat QA scripts unchanged); it is no longer just
  documentation of the sim's numbers, changing it for real now genuinely
  ...
  day→dusk→day cycle via 40 real actions + a sleep, asserting both
