...
- **Dirty/conflict state (2026-07-14, Codex usage-limit interrupt):** Codex hit its limit mid-Wave-2; the Fable session hit QUIESCE (fable band 90%) while landing. Wave 2's uncommitted seam is PARKED on branch `wip/113-wave2-codex` (4e9eaba, 15 files, WIP-tagged, GATES RED — do not merge). `main` working tree is CLEAN at `82887b1`. **`main` is 4 commits ahead of origin, UNPUSHED** (eb15f30..82887b1) — push only with `[ci-full]` after a local full sweep next session.
- **Verification so far:** approved design `eb15f30`; plans `2a1e3b5`; audit `c0d1a61`; 23 real windowed canonical/probe/gallery runs PASS across Waves 0-1 with personally judged frames. Focused sprite/combat tests, `load_gate`, and canonical `crab_cull_loop`/`cisterns_fight` green; temporary QA capture hooks removed.
- **Wave 2 gate results (on the parked seam, 2026-07-14):** `load_gate` PASS; 22/24 unit suites PASS. Two RED, both diagnosed on the WIP branch:
  1. `test_combat_visuals` — Codex's statue/rubble arena-decor swap landed in **`deep_warren`**, not **`ruin_court`** (the test asserts ruin_court; on disk ruin_court still has two boulders, deep_warren got `dungeon_statue`/`dungeon_rubble`). Fix = move the decor edit to the ruin_court block (and decide whether deep_warren keeps or reverts its swap).
  2. `test_sim_core` — the new Eloise `[5,8]`/`facing down` assert fails even though `witch_hollow.json` on disk is correct. Tests read `WISceneCatalog.compose()`, not the map file directly — UNDIAGNOSED; suspect compose() reads a composed/frozen source that needs regeneration, or a type mismatch in the new assert. Diagnose before touching data.
- **Not yet run on the seam:** the five touched canonicals (`deep_descent`, `riverfarm_talk`, `ruin_walkthrough`, `trader_earn_loop`, `upstairs_walkthrough` — all seed 9), balance harness (arenas.json decor touched), full sweep, windowed before/after evidence for every Wave 2 fix, reviewer pass. VISUAL-LOG entries for Wave 2 surfaces remain unchecked (correct — no evidence yet).
- **Operator need / next action (fresh session):** check `scripts/usage_status.sh` first. Then: (1) checkout `wip/113-wave2-codex`, fix the two red gates above, (2) run the full Wave 2 gate ladder (units → 5 canonicals seed 9 → balance harness → full sweep → windowed reads you judge yourself), (3) gated merge to main, tick the VISUAL-LOG entries with evidence, reviewer pass, (4) push everything with `[ci-full]`.
