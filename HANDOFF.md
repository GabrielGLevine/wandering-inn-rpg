...
## 🎯 NEXT SESSION FIRST ACTION (2026-07-20 early: ART WAVE FULLY SHIPPED)
The entire c-lane art wave + follow-ups MERGED tonight, all user
FEEL-approved: PR #265 (#198 dusty_scroll split + sweep), #266 (stalls/
benches/statue + Pallass de-clone), #267 (#224 batch B + ALL FOUR rigs:
Erin canon-corrected t-shirt/jeans, Rags bespoke, Ceria full re-rig
WITH cast, Ruin Warden combat rig). PR #268 (pantry-door consolidation
+ the scroll's earned decipher — the `skill_uses` per-skill arm sim
extension) merging on CI. PixelLab MCP total spend ≈ $2.30, ~$2.7
credits left.
1. **Verify PR #268 merged**; then the only open c-lane item is #222's
   bookkeeping (comment-close it: chessboard proved nonexistent, tells
   pre-wired, everything else shipped across #265-267).
2. **v0.13 release cut when ready**: #111's two manual rehearsals gate
   the tag (docs/design/2026-07-19-111-rename-release-checklist.md).
3. NEW TRAPS on main (wi-verifying-changes): new-producer-key rule
   (extend ALL catalog walks; grep the sibling key), visual_states
   `{when:{counter,at}}` contract, variants override via
   `accomplishment`.
USER-GATED: #195 audio listen; #211 leveling feel; 3 Rags reads;
#134/#141 lore-gated.
