...
## 🏁 v0.6.0 MILESTONE + FABLE SESSION CLOSE (2026-07-13)

**The depth wave drained to a milestone under the budget endgame** (user: "<10% Fable — guide to milestone release and stop"). Shipped in v0.6.0 on top of v0.5.2: **five CC0 ambient beds** on a layout Ambience bus (inn murmur/street/floodplains/dungeon hum/sewers drip — the maps have ears now that web audio works; every file astats-verified non-silent), **the Combat Speed setting** (Normal/Fast/Instant + settings row, pin-proven) and **the guarded plain-sleep skip** (consolidation offers stay unskippable), **standing delivery routes + post_game repeatable postings** (the hour-4 loop's data heart; rotation windows re-derived, gated appends byte-identical pre-post_game), **boss-unique drops** on the three dungeon bosses (existing loot machinery). Composed gate: 25/25 units (test_sleep_veil new), 101-cell harness, 109-script sweep, web audio-output 0.22 RMS. Freeze list cut at 0.6.0 (22/68/51/21/336).
...
5. #102/#103 context economy (content-quiet window), #94 capability sub-lanes, #19 (user), the localization + demo-boundary decisions (user).

**USER queue standing**: itch mobile-friendly flag (v0.5.2+ carries full touch); localization posture + demo boundary; #19 Steam secrets.

**DEPLOY CONFIRMATION (session close): itch 0.6.0 LIVE (butler attempt 2 through the IPv6 flakiness), Pages redeployed green, mac v0.6.0 zip delivered.** CI on the landing push runs under [ci-full]; the sweep-smoke job guards every future push. This was the final Fable-orchestrated session of the run — the wi-* skill library + this doc carry the project.

## 🔊 THE WEB-SILENCE BUG: FOUND AND FIXED (2026-07-13, ee5cc61) — v0.5.2

**Every web build ever shipped was silent; native was always fine; the fix is one bus-layout resource.** The user reported Pages silent too (clean console) — that killed the worklet-fetch theory and my earlier "runner artifact" adjudication, which was WRONG. Diagnosis chain, each step evidence-gated: (1) native raw-engine probe = audible; (2) the real game natively (env-gated WIAudio instrumentation, real windowed QA flow) = SFX and music both hit Master — first output-level verification in the project's history, corroborated by the user's mac-build report; (3) an AnalyserNode tap in the web runner = the game's graph renders ALL ZEROS while a control oscillator through the same tap sings (0.70 RMS) — silence reproduced locally, honestly; (4) vanilla 4.7 single-thread export = audible on Master but SILENT the moment the sound routes through a bus created at runtime via AudioServer.add_bus; a bus defined in default_bus_layout.tres = audible. WIAudio built Music/SFX/UI/Voice at runtime and routes everything through them. **Fix**: default_bus_layout.tres (exact graph, UI→SFX, rest→Master), _setup_buses stays as the idempotent fallback. Game web build measured 0.19 peak RMS. **The tooth that was always missing**: the runner now measures REAL rendered output every run (tap + oscillator control); WI_REQUIRE_AUDIO_OUTPUT=1 hard-fails a silent graph — wired into CI's web-parity job. LESSON (the deepest instance yet of the passing-tests gotcha): every audio tooth asserted bus VALUES and worklet FETCHES; nobody had ever measured a rendered sample on any platform. Deployed: **v0.5.2 to itch + Pages redeploy** (both carrying the fix). SELF-INFLICTED DETOUR logged: a mid-diagnosis "regression" in defeat_reload was my own re-run omitting its manifest seed (--seed=1), and a git stash -u run violated the overlay-blindness rule — both recognized, neither contaminated the verdict.

## 🌐 GITHUB PAGES LIVE (2026-07-13) — closed #108; ACTIONS BILLING FIXED

**https://gabrielglevine.github.io/wandering-inn-rpg/** — live and probed (overlay marker + wasm/pck serving). The user allocated $5 to Actions (public-repo minutes are free; the allocation unblocked the account) and directed the Pages release. Shape: `pages.yml`, workflow_dispatch ONLY (budget-respecting, secrets unreachable from forks), artifact flow (`actions/deploy-pages` — NEVER a gh-pages branch: the pck bakes licensed assets and a branch would put them in public git history forever), mirrors release.yml's pinned-Godot overlay+export, gated by import + smoke boot + the #101 smoke tier with real assets. Pages build_type set to `workflow` via API. Redeploy = `gh workflow run pages.yml`. CI is ALIVE again — 39c5ef5's push ran green (units+leak+smoke+sweep-smoke).

## 🔬 ENGINE-SOURCE ADJUDICATIONS (2026-07-13, user-sanctioned 4.7 source reads — /tmp/godot-src)

Four shipped-on-inference claims settled from source: **(1) the #106 virtual-keyboard chain is CORRECT end-to-end** — preset flag → `experimentalVK` → `GodotConfig.virtual_keyboard`; JS gates on the flag AND `'ontouchstart' in window` (touch devices only, desktop unaffected); LineEdit summons on FOCUS_ENTER only when focus wasn't via arrow keys (line_edit.cpp:1677-1683 — the QA keyboard path is structurally safe, exactly as the lane claimed). Phone name-entry will work. **(2) `emulate_mouse_from_touch` defaults TRUE** (main.cpp:3666) — the mobile spec's foundation holds. **(3) Web audio has NO automatic fallback when the worklet MODULE fetch fails** — `godot_audio_has_worklet` checks API presence only (`ctx.audioWorklet`), so the ScriptProcessor fallback engages only on ancient browsers; a failed `index.audio.worklet.js` fetch on a modern browser = created-but-silent driver. **This makes the itch total-silence report engine-explicable**: the user's console check (a failed request to index.audio.worklet.js) is exactly the decisive test. **(4) First-interaction audio resume is engine-handled** — key/mouse/touch/IME callbacks all resume the AudioContext (display_server_web.cpp:189/306/766/894).

## 🎁 THE PAYOFF WAVE SHIPPED (2026-07-13) — closed #88 #89 #94 #104

Three lanes, all opus-reviewed with fix waves. **#88**: defeat now rewinds EXACTLY the fight — including the pre-fight dialogue choice (the review caught the snapshot capturing same-option accomplishments; the fix emits PRE_COMBAT_CHOICE before the effects loop, with an armed-skip against the post-effects overwrite and a refusal disarm; the single-verb-per-effect-dict convention is now an enforced validator); New Game overwrite-confirms (destructive-safe cursor on No, TestDriver bypass pinned); auto→auto_prev rotation; the classless_solo floor cell measured (0.07 — visibility, not a target); relc_descent_rewind = the canonical proving the rewind erases relc_joined_descent and a clean re-choice fields no Relc. **#89**: every orphaned counter consumed — went_alone (Relc's flagship variant), act_iv gated on door_awakened (the review caught the seal-only gate erasing Act III's payoff; fixed per charter, both canonical pins restored verbatim), the Wilovan fence (attested pricing, no-haggle fiction endorsed, retires via the witch precedent, end-to-end QA leg), the second-door mini-arc (mirrored study-sleeps hook, dungeon_depths portal row riding the has_map guard), Horns Social II (flavor-only perks — CHOICES 33; Ceria's Wistram rail honored). **#94+#104**: the same-map presence ghost is FIXED (a PHASE_CHANGED reconciler with a negative-controlled tooth + light-unregister discipline + a wiring tripwire suite, test_world_visuals); 6 archer/rogue icons + a both-directions iconless tripwire; VISUAL-LOG 32→20 (every closure git-verified, every survivor names its blocking capability); the razorbeak arena re-point surfaced a real #97 bug (training_yard's trivial flag was silently suppressing class-progression tallies on wild culls — flipped on, disclosed). **Composition caught two more seams**: the classless rewind fixture violated fixture-coherence (warrior:1 fix, loss still deterministic) — the lane's "full battery" claim hadn't run that suite; and the relc rewind × went_alone cross-lane contradiction was caught at REVIEW time and fixed pre-merge (the first seam caught before composition — the plan's seam map working). Composed gate: 23/23 units, 101-cell harness, 109-script sweep. Canonicals 109 + 1 peek utility; harness 101 cells.

## ⚔️ THE COMBAT WAVE SHIPPED (2026-07-13) — closed #90 #97

Two lanes, both opus-reviewed FIX-FIRST → fixed → verified. **#90**: four statuses (burning EOT tick-then-purge / weakened out×0.75 / guarded in×0.75 / rooted pool-0+dash-refuse with ACTION_REFUSED legibility), riders on 4 enemy skills incl. the new enemy-only guarding_ward, area_skill + support_skill AI arms (goblin_shaman→icy_floor, rift_vermin_c→flame_pillar, ruin_ward_b→guarding_ward), STATUS_TICKED presentation, the invisibility tooth, shaman_zone_loop = the enemy-cast-terrain canonical (organic geometry proven unreachable by a 200-fight probe → combat_set_cells QA affordance forces it, fail-loud). Two shipped-stat changes per the #83 playbook — CHOICES 31. **#97**: 6 wiki-verified species / 9 records across all four regions (Corusdeer/Razorbeak/Mothbear/Kingslayer Spider/2 golem roles), varied AI profiles, crab-nest-shape consent conversations (voice-passed clean), posting pool 20→26, 4 new canonicals + bestiary_peek utility; golem names re-ruled to attested "Stone Golem" — CHOICES 32; 3 VISUAL-LOG entries (golem/mothbear overlap, razorbeak read, training_yard arena). **The composed rehearsal earned its keep**: corusdeer's flame_bolt inherited #90's burning rider live (7 applications first run) — two measured cells dropped honestly (corusdeer 0.77→0.65, forge_golem 0.67→0.49/0.81→0.71) and the floodplains canonical's second leg died to a burn tick at its rider-inert rng state → re-derived via _derive_rng_state (story in the fixture). Composed gate: 22/22 units, 99-cell harness, 108-script sweep, zero grep hits. Sweep now 108 canonicals; harness 99 cells.
