...
**RUNNING: lane-8dc1 FIX-FIRST wave.** Base commit 70a1661 (quest + C4 + D1 + 3 delve canonicals, 90/90 on its tree; copy shipped verbatim; C4 cost = ally_hp_penalty riding the generic hp_mod — disclose-and-proceed adjudicated DEFENSIBLE by the sim-seams reviewer). 3-dimension review workflow (sim-seams MERGE / content-voice FIX-FIRST / qa-teeth MERGE; 2 Importants adversarially CONFIRMED): (1) solo-vault ordering accident — party gate added to encounter_when; (2) the Ksmvr-penalty cost had zero snapshot teeth — delve_talk pins max_hp==17. Also taken: hp_mod fold clamp, counter-id existence validator, penalized-party harness cell, snare-roster re-derive (CHOICES 17: FIGHT-route cost was illusory at 0.99/2r with non-persisting HP — roster-only fix aiming gated band), Yvlon _comment canon fix. Deviations 1-3 (ally-gate id, trigger_radius drop, dart-slit convergence id) adjudicated FINE by reviewers.
