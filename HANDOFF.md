...
Last updated: 2026-07-07 (**v0.3.0 LIVE ON ITCH** — tag f73433b, release.yml green, butler build #1779516. Planning is GITHUB-NATIVE: issues/milestones/labels at github.com/GabrielGLevine/wandering-inn-rpg — read the board, not a roadmap doc. Playtest Interlude COMPLETE: #2 UI wave, #3 Relc figure-height retune, #4 em-dash corpus pass (155 pairs; escape-variant grep lesson in wi-adding-dialogue-and-quests), #5 PixelLab art wiring (canon red-hair Lyonette, drake patron, bespoke sign/board/bench; new VISUAL-LOG item: runner_board grass-base indoors) — all closed via landing-commit syncs; milestones 2+3 closed. **Chain rung: 8a gates on issue #7 (USER SESSION — Door/Garden earn conditions; canon-checked options prepped in the issue comments).** While that waits, unblocked honest-debt/polish lanes running: #27 Vess rotation bark + #28 dark-arena legibility. User queue: #7 session, taste issues #32-#35, `gh auth refresh -s project,read:project` for the Projects board. 56 canonicals green through every close.)
