...
| `assert_dialogue_displayed` | `contains` | pins the rendered line panel: visible, on screen, non-empty, AND not occluded (#509: fails when a visible panel on an equal-or-higher canvas layer covers >=20% of it; the measurement lands in events.jsonl as `qa_dialogue_display_probe`) |
| `touch_screen`/`touch_cell`/`touch_title_row`/`touch_dialogue_option`/`touch_field_chip`/`touch_settings_row`/`touch_purchase_row` | `pos:[x,y]` / `cell:[x,y]` / `row` (1-based) / `option` (1-based) / `chip` / `row` (1-based) / `row` ("buy"/"cancel") | #503 REAL-TOUCH TIER: natively an emulated click labelled `qa_touch {real:false}`; on the web runner with `--touch` the driver publishes the WINDOW pixel and Playwright performs a genuine `page.touchscreen.tap` (`qa_touch {real:true}`); an unserviced request FAILS the step -- no keyboard/mouse fallback. Canonical: `mobile_touch_smoke` (native = emulated; `run_web_qa.sh mobile_touch_smoke 9 --skip-export --touch --device=iphone --portrait-entry` = the real-tap run, still EMULATED hardware). #253: `save_port_web[_cancel|_invalid]` are WEB-ONLY (unregistered; CI's web job runs them with `--import-cancel` / `--import-file=`) -- the runner captures Export downloads and answers the Import file chooser |
